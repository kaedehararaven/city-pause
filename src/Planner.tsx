...
function planText(plan: Recommendation) {
  ...
  return [
    ...
    `总耗时 ${plan.totalMinutes} 分钟（含 3 分钟缓冲），预算剩余 ${plan.remainingMinutes} 分钟。`,
...
export function Planner({
  ...
  const changed = () => { if (plans !== null) setStale(true); setSelected(null); setError(""); };
  function generate(event?: FormEvent) {
    ...
    if (dataMode === "real" && !realData) {
      setError("REAL 数据尚未就绪。请先在页面底部完成真实定位、POI 与当前模式所需路线查询。");
      return;
    }
    ...
    setPlans(buildRecommendations(intent, dataMode === "real" ? realData! : mockData));
    ...
    <main>
      ...
      <div className="workspace"><section id="planner-input" className="input-panel" aria-labelledby="inputTitle"><div className="section-top"><span className="eyebrow">01 / 说说你的现在</span><span className="mini">随时可以调整</span></div><h2 id="inputTitle">这段时间，想怎么过？</h2>
        <form onSubmit={generate} noValidate>
          ...
          {error && <p className="error" role="alert">{error}</p>}<button type="submit" className="primary">看看我的暂停方案 <span>→</span></button><p className="fine">{dataMode === "mock" ? "本模式使用规则匹配 · 地点与时间均为虚构演示" : realStatus === "ready" ? "本模式使用 REAL POI 与当前模式所需的真实路线 · 停留和缓冲为推荐规则" : "REAL 数据未就绪，不会偷偷使用 Mock 替代"}</p>
        </form>
      </section>
      <section className="results" aria-labelledby="resultsTitle"><div className="section-top"><span className="eyebrow">02 / 你的时间，你来选</span><span className="mini">{plans === null ? "等待你的灵感" : `${plans.length} 个可行方案`}</span></div><h2 id="resultsTitle">{plans === null ? "一小段空闲，也有很多可能。" : plans.length ? `给这 ${lastIntent?.availableMinutes} 分钟，一点好安排。` : "这次的条件，暂时安排不下。"}</h2>
        ...
        {plans === null ? <div className="empty"><div className="empty-symbol" aria-hidden="true">Ⅱ</div><h3>今天，想按下哪一种暂停？</h3><p>选好时间，我们会安排步行和停留，默认无需返回起点。<br/>不用赶路，也不用把每一分钟填满。</p><div className="possibilities"><span>歇一会儿</span><span>走一小圈</span><span>发现附近</span></div></div> : plans.length ? plans.map((plan, index) => <article key={plan.id} className={`plan${selected?.id === plan.id ? " selected" : ""}`}><div className="plan-top"><span className="plan-badge">{plan.source.toUpperCase()} · 0{index+1} / {plan.strategyLabel}</span><span className="total">{plan.totalMinutes}<small>分钟</small></span></div><h3>{plan.title}</h3><p className="route">{plan.originName} → {plan.places.map(place => place.name).join(" → ")}{(plan.returnMode === "return_to_start") ? ` → ${plan.originName}` : ""}</p><div className="metrics"><span>步行 {plan.walkingMinutes} 分</span><span>停留 {plan.stayMinutes} 分</span><span>缓冲 3 分</span><span>{plan.costStatus === "required" ? "需要消费" : plan.costStatus === "not-required" ? "无需消费" : "消费信息未知"}</span></div><p className="reason">{plan.strategyReason}；{(plan.returnMode === "return_to_start") ? "已计入返程" : `终点是${plan.places.at(-1)?.name}`}，余量 {plan.remainingMinutes} 分钟。</p><div className="plan-actions"><details><summary>查看时间安排</summary><ol className="timeline">{(() => { let elapsed = 0; return plan.steps.map((step, stepIndex) => { const start = elapsed; elapsed += step.minutes; return <li key={stepIndex}><small>第 {start}～{elapsed} 分钟 · {step.kind}</small>{step.label}</li>; }); })()}</ol></details><button type="button" className="select" disabled={stale} onClick={() => choose(plan)}>就选这个 ↗</button></div></article>) : <div className="no-result"><h3>留一点更充裕的时间吧。</h3><p>在当前时间、地点和返程条件下，没有可行方案。可以增加可用时间后重试；我们会保留你的消费与地点限制。</p></div>}
