...
旧单公园诊断代码继续保留，但已不再给 Planner 提供数据。A2 SearchPolicy 驱动五类别发现，用户点击生成 REAL 计划时，再对有限候选请求真实路线并进入推荐核心。发现层自身不自动请求路线。当前联调与动态停留验证见 REAL_PLAN_INTEGRATION；不得将旧双向路线记录误作本次证据。
